...
function Header() {
  ...
  const getCollateral = async () => {
    ...
    dispatch(
      VaultActions.updateCollateral(ethers.utils.formatEther(collateralBalance))
    );
  };
          ...
          <HeaderButtonsContainer>
            ...
            <Dropdown>
              <WalletInfo variant='none'>
                <WalletAddress>{shrinkAddress(account)}</WalletAddress>
                <BalanceInfo>
              ...
              <HeaderDropdownMenu>
                <Dropdown.Item href='#' onClick={() => modalShowHandler()}>
                  FTM/WFTM Station
                </Dropdown.Item>
                <Dropdown.Divider />
          ...
          >
            ...
            <Modal.Body>
              <WrapBoxWrapper className={wrapUnwrap ? '' : 'reverse-direction'}>
                <WrapBoxRow>
                  <WrapBoxLabel>FTM</WrapBoxLabel>
                  <WrapBoxInputWrapper>
                    <WrapBoxBalance>
                      <WrapBoxBalanceAmount>
                        Balance: {formatFTMBalance(ftmBalance)}
                      </WrapBoxBalanceAmount>
                      {wrapUnwrap && (
                        <WrapBoxInputMax
                          onClick={() => setWrapAmount(ftmBalance)}
                        >
                          (Max)
                        </WrapBoxInputMax>
                      )}
                    ...
                    <WrapAmountPrice>${wrapPrice()}</WrapAmountPrice>
                  </WrapBoxInputWrapper>
                </WrapBoxRow>
                <WrapConvertButtonRow>
                  <WrapConvertButton onClick={() => changeWrapConertHandler()}>
                    <WrapImg src={SwapIcon} />
                  </WrapConvertButton>
                </WrapConvertButtonRow>
                <WrapBoxRow>
                  <WrapBoxLabel>WFTM</WrapBoxLabel>
                  <WrapBoxInputWrapper>
                    <WrapBoxBalance>
                      <WrapBoxBalanceAmount>
                        Balance: {formatFTMBalance(wFTMBalance)}
                      </WrapBoxBalanceAmount>
                      {!wrapUnwrap && (
                        <WrapBoxInputMax
                          onClick={() => setWrapAmount(wFTMBalance)}
                        >
                          (Max)
                        </WrapBoxInputMax>
                      )}
              ...
              <WrapBoxRow>
                <WrapButton
                  onClick={() => wrapButtonClickHandler()}
                  disabled={
                    wrapAmount === '' ||
                    wrapping ||
                    (wrapUnwrap && wrapAmount * 1.0 > ftmBalance) ||
                    (!wrapUnwrap && wrapAmount * 1.0 > wFTMBalance)
                  }
                >
